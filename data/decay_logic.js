...
function calculateDynamicBaseDecayRate(dataSet) {
    ...
    let normalizedQTc = normalize(dataSet.ecg.qtcInterval, minMaxValues.qtcInterval.min, minMaxValues.qtcInterval.max);
    let normalizedCreatinine = normalize(dataSet.labs.creatinine, minMaxValues.creatinine.min, minMaxValues.creatinine.max);
    let normalizedEGFR = normalize(dataSet.labs.eGFR, minMaxValues.eGFR.min, minMaxValues.eGFR.max);
    let normalizedGlucose = normalize(dataSet.labs.glucose, minMaxValues.glucose.min, minMaxValues.glucose.max);
