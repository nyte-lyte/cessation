...
// itself compiled in, or a piece knows its siblings before they are inscribed.
{
  ...
  r.checks++;
  if (!bundle.includes('let healthDataSets = [];')) {
    r.fail(ctx, 'the empty healthDataSets shell is missing from the bundle — build.js may have gone back to inlining the data file');
  }
}
...
function bootEngine({ serveMetadata }) {
  ...
  globalThis.fetch = (url) => {
    ...
    if (url === `/r/metadata/${OWN_ID}`) {
      return serveMetadata ? ok(metadataHex) : Promise.reject(new Error('404'));
    }
