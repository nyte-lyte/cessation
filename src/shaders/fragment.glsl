...
void main(){
    ...
    // Cross-side pulls — left (rhythm+renal) ↔ right (axis+electrolyte)
    // Strength = abA * abB * 0.10 — both must be abnormal for significant pull
    float pullScale = 0.10;

    // QTc ↔ tAxis — both measure repolarization, different angles
    float pullQTcTA = abQTc * abTA * pullScale;
    vec2 dirQTcTA = normalize(fTA - fQTc + vec2(1e-5));
    fQTc += dirQTcTA * pullQTcTA;
    fTA  -= dirQTcTA * pullQTcTA * 0.5;

    // ventRate ↔ potassium — K dysregulation drives arrhythmia
    float pullVRK = abVR * abK * pullScale;
    vec2 dirVRK = normalize(fK - fVR + vec2(1e-5));
    fVR += dirVRK * pullVRK;
    fK  -= dirVRK * pullVRK * 0.5;

    // eGFR ↔ QRS-T angle — kidney failure → uremic cardiomyopathy → electrical dissonance
    float pullEGFAng = abEGF * abAng * pullScale;
    vec2 dirEGFAng = normalize(fAng - fEGF + vec2(1e-5));
    fEGF += dirEGFAng * pullEGFAng;
    fAng -= dirEGFAng * pullEGFAng * 0.5;

    // BUN ↔ CO2 — uremia drives metabolic acidosis, both rise together
    float pullBUNCO2 = abBUN * abCO2 * pullScale;
    vec2 dirBUNCO2 = normalize(fCO2 - fBUN + vec2(1e-5));
    fBUN += dirBUNCO2 * pullBUNCO2;
    fCO2 -= dirBUNCO2 * pullBUNCO2 * 0.5;
